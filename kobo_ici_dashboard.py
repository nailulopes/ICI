...
QUALITY_MAP    = {"EN":{5:"Very good",4:"Good",3:"Neutral",2:"Poor",1:"Very bad",0:"Don't know"},
                  "FR":{5:"Très bien",4:"Bien",3:"Neutre",2:"Mauvais",1:"Très mauvais",0:"Ne sait pas"}}
QUALITY_ORDER  = {"EN":["Very bad","Poor","Neutral","Good","Very good","Don't know"],
                  "FR":["Très mauvais","Mauvais","Neutre","Bien","Très bien","Ne sait pas"]}
DECISIONS_MAP  = {"EN":{1:"Included + enough info",2:"Included, not enough info",3:"Sometimes included",4:"Not included",0:"Don't know"},
                  "FR":{1:"Incluse + info suffisante",2:"Incluse, info insuffisante",3:"Parfois incluse",4:"Non incluse",0:"Ne sait pas"}}
EPI_MAP        = {"EN":{1:"Yes, with consent",2:"Yes, without consent",3:"No — declined",4:"No — not recommended"},
...
LIKERT_QS      = {"EN":{"introduction":"Staff introduced themselves","spoke":"Staff spoke clearly",
                         ...
                         "positive":"Providers encouraging","morale":"Staff well-equipped","coop":"Coordinated care"},
                  "FR":{"introduction":"Le personnel s'est présenté","spoke":"Personnel parlait clairement",
                         "communication":"Ouvert aux questions","privacy":"Vie privée protégée",
                         "respect":"Traitée avec respect","values":"Croyances et choix respectés",
                         "positive":"Prestataires encourageants","morale":"Personnel bien équipé","coop":"Soins coordonnés"}}
EMOTION_LABELS = {"EN":{1:"Capable",2:"Incapable",3:"Anxious",4:"Supported",5:"Exhausted",6:"Active",
...
INFO_LABELS    = {"EN":{1:"Caring for my new baby",2:"Family planning advice",
                         3:"Warning signs needing consultation",4:"Where to go for follow-up"},
...
POSITIVE_EMO   = {"EN":{"Capable","Supported","Active","Relaxed","Responsible","Secure"},
...
def load_data():
    ...
    while url:
        ...
        if r.status_code != 200:
            st.error(f"Kobo API error {r.status_code}"); return pd.DataFrame()
        data = r.json(); results.extend(data.get("results",[])); url = data.get("next")
...
def prep(df, lang):
    ...
    for col, mp in [("method",METHOD_MAP[lang]),("education",EDUCATION_MAP[lang]),("risk",RISK_MAP[lang]),
        ...
        if col in df.columns:
            df[col]=to_int(df[col]); df[col+"_label"]=df[col].map(mp).fillna("?")
    for col in LIKERT_QS[lang]:
        if col in df.columns:
            df[col]=to_int(df[col]); df[col+"_label"]=df[col].map(LIKERT5_MAP[lang]).fillna("?")
    if "skin" in df.columns:
        df["skin_int"]=first_token_int(df["skin"]); df["skin_label"]=df["skin_int"].map(SKIN_MAP[lang]).fillna("?")
...
# ════════════════════════════════════════════════════════════
# HELPER: make a clean plotly figure layout
# ════════════════════════════════════════════════════════════
def clean_layout(fig, title="", height=280, legend_below=False):
    """Apply consistent clean layout to any figure, title always above chart."""
    layout = dict(
        title=dict(text=title, font=dict(size=13, family="DM Serif Display, serif", color="#1a1a1a"),
                   x=0, xanchor="left", pad=dict(b=8)),
        margin=dict(t=48, b=90 if legend_below else 16, l=8, r=8),
        height=height,
        plot_bgcolor="white", paper_bgcolor="white",
        font=dict(family="DM Sans, sans-serif", size=11),
    )
    if legend_below:
        layout["legend"] = dict(orientation="h", y=-0.25, x=0, font=dict(size=10))
...
with st.spinner("Loading data..." if lang=="EN" else "Chargement..."):
    raw = load_data()
if raw.empty:
    st.warning("No data." if lang=="EN" else "Aucune donnée."); st.stop()
df = prep(raw, lang)
...
if logo_b64:
    st.sidebar.markdown(f'<img src="data:image/png;base64,{logo_b64}" style="width:170px;margin-bottom:8px">', unsafe_allow_html=True)
...
logo_overlay = f'<img class="hero-logo" src="data:image/png;base64,{logo_b64}">' if logo_b64 else ""
st.markdown(f"""
<div class="hero">
  {logo_overlay}
  ...
  <div class="hero-stats">
    ...
    <div class="hero-stat hero-stat-bad">
      <div class="hero-stat-num">{epi_nc}</div>
      <div class="hero-stat-label">{t("kpi_epi",lang)}</div>
    </div>
  </div>
</div>
...
if "_submission_time" in df.columns and df["_submission_time"].notna().any():
    ...
    fig.update_traces(line_width=2, fillcolor=f"rgba(0,158,115,0.12)")
...
if "no_deliveries" in df.columns:
    ...
    nd.columns=["n","count"]; nd["n"]=nd["n"].astype(int).astype(str)
    fig=px.bar(nd, x="n", y="count", color_discrete_sequence=[SKY],
...
if rows:
    ...
    fig.update_layout(
        ...
        margin=dict(t=8,b=100,l=8,r=8), height=460,
        plot_bgcolor="white", paper_bgcolor="white",
        font=dict(family="DM Sans, sans-serif"))
    st.plotly_chart(fig, use_container_width=True)
...
if "expect_label" in df.columns and "satisfaction_label" in df.columns:
    ...
    for col,field,title in [(c1,"expect_label",t("sat_expect",lang)),(c2,"satisfaction_label",t("sat_actual",lang))]:
        vc=df[field].value_counts().reindex(q_order, fill_value=0).reset_index(); vc.columns=["r","n"]
...
if "emotion" in df.columns:
    emo_labels=EMOTION_LABELS[lang]; pos=POSITIVE_EMO[lang]
    ...
    fig.update_layout(
        ...
        margin=dict(t=8,b=70,l=8,r=8), height=420,
        plot_bgcolor="white", paper_bgcolor="white",
        font=dict(family="DM Sans, sans-serif"))
    fig.update_xaxes(gridcolor="#eeeeee"); fig.update_yaxes(showgrid=False)
    st.plotly_chart(fig, use_container_width=True)
