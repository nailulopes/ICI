"""
ICI Women's Experience Dashboard
International Childbirth Initiative
"""

import streamlit as st
import pandas as pd
import plotly.express as px
import requests
import base64
from pathlib import Path
from datetime import datetime

# ─────────────────────────────────────────
# CONFIGURATION
# ─────────────────────────────────────────
ASSET_UID = "aT3kXmLeYLtUC6zVAV5abW"
BASE_URL  = "https://eu.kobotoolbox.org"
try:
    KOBO_TOKEN = st.secrets["KOBO_TOKEN"]
except Exception:
    KOBO_TOKEN = ""
    st.error("⚠️ KOBO_TOKEN not found. Add it to Streamlit Secrets.")
    st.stop()
# ─────────────────────────────────────────

# ── Bilingual labels ──
LABELS = {
    "title":          {"EN": "Women's Experience Dashboard", "FR": "Tableau de bord — Expérience des femmes"},
    "caption":        {"EN": "International Childbirth Initiative · 12 Steps to Safe and Respectful MotherBaby-Family Maternity Care",
                       "FR": "Initiative Internationale pour la Naissance · 12 étapes pour des soins de maternité sûrs et respectueux"},
    "filters":        {"EN": "Filters", "FR": "Filtres"},
    "date_range":     {"EN": "Date range", "FR": "Période"},
    "birth_method":   {"EN": "Birth method", "FR": "Mode d'accouchement"},
    "high_risk":      {"EN": "High-risk pregnancy", "FR": "Grossesse à risque"},
    "filtered":       {"EN": "Filtered responses", "FR": "Réponses filtrées"},
    "refresh":        {"EN": "🔄 Refresh data", "FR": "🔄 Actualiser"},
    "overview":       {"EN": "Overview", "FR": "Vue d'ensemble"},
    "total":          {"EN": "Total responses", "FR": "Total réponses"},
    "positive_rating":{"EN": "Positive care rating", "FR": "Évaluation positive"},
    "skin_kpi":       {"EN": "Immediate skin-to-skin", "FR": "Peau à peau immédiat"},
    "exam_kpi":       {"EN": "Vaginal exams w/o consent", "FR": "Examens vaginaux sans consentement"},
    "epi_kpi":        {"EN": "Episiotomies w/o consent", "FR": "Épisiotomies sans consentement"},
    "timeline":       {"EN": "Responses Over Time", "FR": "Réponses dans le temps"},
    "group_by":       {"EN": "Group by", "FR": "Regrouper par"},
    "month":          {"EN": "Month", "FR": "Mois"},
    "week":           {"EN": "Week", "FR": "Semaine"},
    "day":            {"EN": "Day", "FR": "Jour"},
    "responses":      {"EN": "Responses", "FR": "Réponses"},
    "profile":        {"EN": "Respondent Profile", "FR": "Profil des répondantes"},
    "birth_method_t": {"EN": "Birth method", "FR": "Mode d'accouchement"},
    "age_group":      {"EN": "Age group", "FR": "Groupe d'âge"},
    "education":      {"EN": "Education level", "FR": "Niveau d'éducation"},
    "gest_weeks":     {"EN": "Gestational weeks at birth", "FR": "Semaines de gestation"},
    "likert_title":   {"EN": "Quality of Care — Likert Scales", "FR": "Qualité des soins — Échelles de Likert"},
    "likert_caption": {"EN": "Response distribution across care dimensions (Always → Never)",
                       "FR": "Distribution des réponses par dimension de soins (Toujours → Jamais)"},
    "autonomy":       {"EN": "Autonomy & Consent", "FR": "Autonomie et consentement"},
    "decisions_t":    {"EN": "Included in care decisions", "FR": "Incluse dans les décisions de soins"},
    "exam_t":         {"EN": "Vaginal exam without consent", "FR": "Examen vaginal sans consentement"},
    "epi_t":          {"EN": "Episiotomy", "FR": "Épisiotomie"},
    "treat_t":        {"EN": "Treatments not wanted / not agreed to", "FR": "Soins non souhaités / sans accord"},
    "clinical":       {"EN": "Clinical Practices", "FR": "Pratiques cliniques"},
    "skin_t":         {"EN": "Skin-to-skin contact after birth", "FR": "Peau à peau après la naissance"},
    "bf_t":           {"EN": "Breastfeeding support", "FR": "Soutien à l'allaitement"},
    "induce_t":       {"EN": "Labour induction", "FR": "Déclenchement du travail"},
    "satisfaction":   {"EN": "Satisfaction — Expectations vs. Reality", "FR": "Satisfaction — Attentes vs. Réalité"},
    "expect_t":       {"EN": "Before I came here, I expected care to be:",
                       "FR": "Avant de venir ici, je m'attendais à ce que les soins soient :"},
    "actual_t":       {"EN": "Now, I feel that my care was:", "FR": "Maintenant, j'estime que mes soins étaient :"},
    "emotions":       {"EN": "How Women Felt at the Time of Delivery", "FR": "Ressenti des femmes au moment de l'accouchement"},
    "pct_resp":       {"EN": "% of respondents", "FR": "% des répondantes"},
    "discharge":      {"EN": "Information Provided Before Discharge", "FR": "Informations données avant la sortie"},
    "pct_info":       {"EN": "% who received this information", "FR": "% ayant reçu cette information"},
    "raw_data":       {"EN": "📋 View raw data", "FR": "📋 Voir les données brutes"},
    "download":       {"EN": "⬇️ Download CSV", "FR": "⬇️ Télécharger CSV"},
    "positive":       {"EN": "Positive", "FR": "Positif"},
    "negative":       {"EN": "Negative", "FR": "Négatif"},
}

def t(key, lang):
    return LABELS.get(key, {}).get(lang, key)

METHOD_MAP = {
    "EN": {1:"Vaginal", 2:"Assisted vaginal (forceps/vacuum)", 3:"Elective/planned caesarean",
           4:"Emergency caesarean", 5:"VBAC", 0:"I don't know"},
    "FR": {1:"Vaginal", 2:"Vaginal assisté (forceps/ventouse)", 3:"Césarienne élective/programmée",
           4:"Césarienne d'urgence", 5:"AVAC", 0:"Je ne sais pas"}
}
EDUCATION_MAP = {
    "EN": {1:"None", 2:"Primary", 3:"Secondary", 4:"Higher than secondary"},
    "FR": {1:"Aucune", 2:"Primaire", 3:"Secondaire", 4:"Supérieur"}
}
RISK_MAP = {
    "EN": {1:"Yes", 2:"No", 0:"I don't know"},
    "FR": {1:"Oui", 2:"Non", 0:"Je ne sais pas"}
}
LIKERT5_MAP = {
    "EN": {5:"Always", 4:"Most of the time", 3:"Sometimes", 2:"Rarely", 1:"Never", 0:"I don't know/N.A."},
    "FR": {5:"Toujours", 4:"La plupart du temps", 3:"Parfois", 2:"Rarement", 1:"Jamais", 0:"Je ne sais pas/N.A."}
}
QUALITY_MAP = {
    "EN": {5:"Very good", 4:"Good", 3:"Neutral", 2:"Poor", 1:"Very bad", 0:"I don't know"},
    "FR": {5:"Très bien", 4:"Bien", 3:"Neutre", 2:"Mauvais", 1:"Très mauvais", 0:"Je ne sais pas"}
}
QUALITY_ORDER = {
    "EN": ["Very bad","Poor","Neutral","Good","Very good","I don't know"],
    "FR": ["Très mauvais","Mauvais","Neutre","Bien","Très bien","Je ne sais pas"]
}
DECISIONS_MAP = {
    "EN": {1:"Yes, included + enough information", 2:"Yes, included but not enough information",
           3:"Sometimes I was included", 4:"No, not included in most decisions", 0:"I don't know/N.A."},
    "FR": {1:"Oui, incluse + informations suffisantes", 2:"Oui, incluse mais informations insuffisantes",
           3:"Parfois incluse", 4:"Non, pas incluse dans la plupart des décisions", 0:"Je ne sais pas/N.A."}
}
EPI_MAP = {
    "EN": {1:"Yes, with my consent", 2:"Yes, but not fully explained / no consent",
           3:"No, because I declined", 4:"No, because staff did not recommend it"},
    "FR": {1:"Oui, avec mon consentement", 2:"Oui, mais pas bien expliqué / sans consentement",
           3:"Non, parce que j'ai refusé", 4:"Non, parce que le personnel ne l'a pas recommandé"}
}
EXAM_MAP = {
    "EN": {1:"Never without verbal consent", 2:"Rarely without consent", 3:"Sometimes without consent",
           4:"Frequently without verbal consent", 5:"Always without consent/being asked"},
    "FR": {1:"Jamais sans consentement verbal", 2:"Rarement sans consentement", 3:"Parfois sans consentement",
           4:"Fréquemment sans consentement verbal", 5:"Toujours sans consentement"}
}
TREAT_MAP = {
    "EN": {2:"Yes", 1:"No", 0:"I don't know"},
    "FR": {2:"Oui", 1:"Non", 0:"Je ne sais pas"}
}
BF_MAP = {
    "EN": {1:"No — did not breastfeed", 2:"No — did not need help", 3:"No, even though I needed help",
           4:"Yes, helped but not enough", 5:"Yes, received the help I needed", 0:"I don't know"},
    "FR": {1:"Non — n'a pas allaité", 2:"Non — n'avait pas besoin d'aide", 3:"Non, malgré le besoin d'aide",
           4:"Oui, aidée mais pas suffisamment", 5:"Oui, a reçu l'aide nécessaire", 0:"Je ne sais pas"}
}
SKIN_MAP = {
    "EN": {1:"Yes — immediate", 2:"Yes — not immediate", 3:"Yes — less than 1 hour",
           4:"No", 5:"I don't know", 9:"Baby sent to neonatal unit"},
    "FR": {1:"Oui — immédiat", 2:"Oui — pas immédiat", 3:"Oui — moins d'1 heure",
           4:"Non", 5:"Je ne sais pas", 9:"Bébé transféré en néonatalogie"}
}
INDUCE_MAP = {
    "EN": {1:"No", 2:"Yes", 0:"I don't know"},
    "FR": {1:"Non", 2:"Oui", 0:"Je ne sais pas"}
}
LIKERT_QUESTIONS = {
    "EN": {
        "introduction": "Staff introduced themselves",
        "spoke":        "Staff spoke clearly",
        "communication":"Staff open to questions",
        "privacy":      "Privacy protected",
        "respect":      "Treated respectfully",
        "values":       "Staff respected my beliefs & choices",
        "positive":     "Providers encouraged empowerment",
        "morale":       "Staff had what they needed to do their jobs",
        "coop":         "Staff worked in a coordinated way",
    },
    "FR": {
        "introduction": "Le personnel s'est présenté",
        "spoke":        "Le personnel a parlé clairement",
        "communication":"Le personnel était ouvert aux questions",
        "privacy":      "Vie privée protégée",
        "respect":      "Traitée avec respect",
        "values":       "Le personnel a respecté mes croyances et choix",
        "positive":     "Les prestataires m'ont encouragée",
        "morale":       "Le personnel avait ce qu'il fallait pour bien travailler",
        "coop":         "Le personnel travaillait de façon coordonnée",
    }
}
EMOTION_LABELS = {
    "EN": {1:"Capable", 2:"Incapable", 3:"Anxious", 4:"Supported", 5:"Exhausted", 6:"Active",
           7:"Relaxed", 8:"Passive", 9:"Responsible", 10:"Dependent", 11:"Secure", 12:"Excluded"},
    "FR": {1:"Capable", 2:"Incapable", 3:"Anxieuse", 4:"Soutenue", 5:"Épuisée", 6:"Active",
           7:"Détendue", 8:"Passive", 9:"Responsable", 10:"Dépendante", 11:"En sécurité", 12:"Exclue"}
}
INFO_LABELS = {
    "EN": {1:"Caring for my new baby", 2:"Advice about family planning",
           3:"Warning signs requiring consultation", 4:"Where to go for follow-up care"},
    "FR": {1:"Soins pour mon nouveau-né", 2:"Conseils sur la planification familiale",
           3:"Signes d'alarme nécessitant une consultation", 4:"Où aller pour le suivi"}
}
POSITIVE_EMOTIONS = {
    "EN": {"Capable","Supported","Active","Relaxed","Responsible","Secure"},
    "FR": {"Capable","Soutenue","Active","Détendue","Responsable","En sécurité"}
}
LIKERT_COLORS = ["#1a7f5a","#57bb8a","#f6c344","#f4845f","#d63031","#cccccc"]
QUALITY_COLORS = ["#d63031","#f4845f","#f6c344","#57bb8a","#1a7f5a","#cccccc"]


def get_logo_b64():
    return ""


@st.cache_data(ttl=300)
def load_data():
    headers = {"Authorization": f"Token {KOBO_TOKEN}"}
    url = f"{BASE_URL}/api/v2/assets/{ASSET_UID}/data/?format=json&limit=3000"
    all_results = []
    while url:
        r = requests.get(url, headers=headers)
        if r.status_code != 200:
            st.error(f"Kobo API error: {r.status_code}")
            return pd.DataFrame()
        data = r.json()
        all_results.extend(data.get("results", []))
        url = data.get("next")
    return pd.DataFrame(all_results)


def to_int(series):
    return pd.to_numeric(series, errors="coerce")


def first_token_int(series):
    return series.astype(str).str.split().str[0].pipe(pd.to_numeric, errors="coerce")


def parse_multiselect_counts(series, valid_keys):
    """Parse space-separated multi-select column into per-key counts."""
    counts = {k: 0 for k in valid_keys}
    for val in series.dropna():
        for token in str(val).split():
            try:
                key = int(token)
                if key in counts:
                    counts[key] += 1
            except ValueError:
                pass
    return counts


def prep(df, lang):
    df = df.copy()
    df["_submission_time"] = pd.to_datetime(df["_submission_time"], errors="coerce")

    for col, mapping in [("method", METHOD_MAP[lang]), ("education", EDUCATION_MAP[lang]),
                         ("risk", RISK_MAP[lang]), ("satisfaction", QUALITY_MAP[lang]),
                         ("expect", QUALITY_MAP[lang])]:
        if col in df.columns:
            df[col] = to_int(df[col])
            df[col + "_label"] = df[col].map(mapping).fillna("?")

    for col, label in LIKERT_QUESTIONS[lang].items():
        if col in df.columns:
            df[col] = to_int(df[col])
            df[col + "_label"] = df[col].map(LIKERT5_MAP[lang]).fillna("?")

    if "skin" in df.columns:
        df["skin_int"] = first_token_int(df["skin"])
        df["skin_label"] = df["skin_int"].map(SKIN_MAP[lang]).fillna("?")

    for col, mapping in [("decisions", DECISIONS_MAP[lang]), ("epi", EPI_MAP[lang]),
                         ("exam", EXAM_MAP[lang]), ("bf", BF_MAP[lang]),
                         ("induce", INDUCE_MAP[lang]), ("treat", TREAT_MAP[lang])]:
        if col in df.columns:
            df[col] = to_int(df[col])
            df[col + "_label"] = df[col].map(mapping).fillna("?")

    if "age" in df.columns:
        df["age"] = to_int(df["age"])
        df["age_group"] = pd.cut(df["age"], bins=[0,19,24,29,34,39,99],
                                  labels=["<20","20–24","25–29","30–34","35–39","40+"])
    return df


# ── Page config ──
st.set_page_config(page_title="ICI Dashboard", page_icon="🤱", layout="wide")
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600&family=DM+Serif+Display&display=swap');
html, body, [class*="css"] { font-family: 'DM Sans', sans-serif; }
h1, h2, h3 { font-family: 'DM Serif Display', serif; }
.metric-box { background:#f8f4f0; border-radius:12px; padding:20px; text-align:center; }
.metric-num { font-size:2.2rem; font-weight:700; color:#1a7f5a; }
.metric-label { font-size:0.85rem; color:#666; margin-top:4px; }
.section-title { font-family:'DM Serif Display',serif; font-size:1.3rem;
                 color:#2d2d2d; border-left:4px solid #1a7f5a;
                 padding-left:12px; margin:28px 0 12px 0; }
</style>
""", unsafe_allow_html=True)

# ── Header ──
logo_b64 = get_logo_b64()
col_logo, col_title, col_lang = st.columns([1, 5, 1])
if logo_b64:
    col_logo.markdown(
        f'<img src="data:image/png;base64,{logo_b64}" style="width:100%;max-width:160px;margin-top:8px">',
        unsafe_allow_html=True)
lang = col_lang.radio("🌐", ["EN", "FR"], horizontal=True, label_visibility="collapsed")
col_title.title(t("title", lang))
col_title.caption(t("caption", lang))
st.divider()

# ── Load data ──
with st.spinner("Loading data..." if lang == "EN" else "Chargement des données..."):
    raw = load_data()

if raw.empty:
    st.warning("No data available." if lang == "EN" else "Aucune donnée disponible.")
    st.stop()

df = prep(raw, lang)

# ── Sidebar ──
if logo_b64:
    st.sidebar.markdown(
        f'<img src="data:image/png;base64,{logo_b64}" style="width:180px">',
        unsafe_allow_html=True)
st.sidebar.header(t("filters", lang))

if "_submission_time" in df.columns and df["_submission_time"].notna().any():
    min_d = df["_submission_time"].min().date()
    max_d = df["_submission_time"].max().date()
    dr = st.sidebar.date_input(t("date_range", lang), value=(min_d, max_d), min_value=min_d, max_value=max_d)
    if len(dr) == 2:
        df = df[(df["_submission_time"].dt.date >= dr[0]) &
                (df["_submission_time"].dt.date <= dr[1])]

if "method_label" in df.columns:
    all_opt = "All" if lang == "EN" else "Tous"
    opts = [all_opt] + sorted(df["method_label"].dropna().unique().tolist())
    sel = st.sidebar.selectbox(t("birth_method", lang), opts)
    if sel != all_opt:
        df = df[df["method_label"] == sel]

if "risk_label" in df.columns:
    all_opt = "All" if lang == "EN" else "Tous"
    opts = [all_opt] + sorted(df["risk_label"].dropna().unique().tolist())
    sel = st.sidebar.selectbox(t("high_risk", lang), opts)
    if sel != all_opt:
        df = df[df["risk_label"] == sel]

st.sidebar.metric(t("filtered", lang), len(df))
if st.sidebar.button(t("refresh", lang)):
    st.cache_data.clear()
    st.rerun()


# ═══ PANEL 1 — KPIs ═══
st.markdown(f'<div class="section-title">{t("overview", lang)}</div>', unsafe_allow_html=True)

sat_good   = (df["satisfaction"].isin([4,5])).sum() / len(df) * 100 if "satisfaction" in df.columns else 0
exam_nc    = (df["exam"].isin([2,3,4,5])).sum() if "exam" in df.columns else 0
epi_nc     = (df["epi"] == 2).sum() if "epi" in df.columns else 0
skin_immed = (df["skin_int"] == 1).sum() / len(df) * 100 if "skin_int" in df.columns else 0

c1,c2,c3,c4,c5 = st.columns(5)
for col, num, label in [
    (c1, len(df),              t("total", lang)),
    (c2, f"{sat_good:.0f}%",   t("positive_rating", lang)),
    (c3, f"{skin_immed:.0f}%", t("skin_kpi", lang)),
    (c4, exam_nc,              t("exam_kpi", lang)),
    (c5, epi_nc,               t("epi_kpi", lang)),
]:
    col.markdown(f'<div class="metric-box"><div class="metric-num">{num}</div>'
                 f'<div class="metric-label">{label}</div></div>', unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)


# ═══ PANEL 2 — Timeline ═══
st.markdown(f'<div class="section-title">{t("timeline", lang)}</div>', unsafe_allow_html=True)
if "_submission_time" in df.columns and df["_submission_time"].notna().any():
    freq_labels = [t("month", lang), t("week", lang), t("day", lang)]
    freq = st.radio(t("group_by", lang), freq_labels, horizontal=True)
    fmap = {t("day", lang):"D", t("week", lang):"W", t("month", lang):"ME"}
    ts = df.set_index("_submission_time").resample(fmap[freq]).size().reset_index(name="n")
    fig = px.area(ts, x="_submission_time", y="n",
                  labels={"_submission_time":"", "n": t("responses", lang)},
                  color_discrete_sequence=["#1a7f5a"])
    fig.update_traces(line_width=2, fillcolor="rgba(26,127,90,0.15)")
    fig.update_layout(margin=dict(t=10,b=10), height=220, plot_bgcolor="white", paper_bgcolor="white")
    st.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 3 — Profile ═══
st.markdown(f'<div class="section-title">{t("profile", lang)}</div>', unsafe_allow_html=True)
c1,c2,c3 = st.columns(3)

if "method_label" in df.columns:
    mc = df["method_label"].value_counts().reset_index(); mc.columns=["m","n"]
    fig = px.pie(mc, names="m", values="n", hole=0.45,
                 color_discrete_sequence=px.colors.qualitative.Safe)
    fig.update_layout(title=t("birth_method_t", lang), margin=dict(t=40,b=10), height=280)
    c1.plotly_chart(fig, use_container_width=True)

if "age_group" in df.columns:
    ac = df["age_group"].value_counts().sort_index().reset_index(); ac.columns=["f","n"]
    fig = px.bar(ac, x="f", y="n", color_discrete_sequence=["#2563eb"],
                 labels={"f": t("age_group", lang), "n":""})
    fig.update_layout(title=t("age_group", lang), margin=dict(t=40,b=10), height=280, plot_bgcolor="white")
    c2.plotly_chart(fig, use_container_width=True)

if "education_label" in df.columns:
    ec = df["education_label"].value_counts().reset_index(); ec.columns=["e","n"]
    fig = px.bar(ec, x="n", y="e", orientation="h", color_discrete_sequence=["#7c3aed"],
                 labels={"e":"","n":""})
    fig.update_layout(title=t("education", lang), margin=dict(t=40,b=10), height=280, plot_bgcolor="white")
    c3.plotly_chart(fig, use_container_width=True)

if "weeks" in df.columns:
    wk = to_int(df["weeks"]).dropna()
    fig = px.histogram(wk, nbins=20, color_discrete_sequence=["#f59e0b"],
                       labels={"value": t("gest_weeks", lang), "count":""})
    fig.update_layout(title=t("gest_weeks", lang), showlegend=False,
                      margin=dict(t=40,b=10), height=220, plot_bgcolor="white")
    st.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 4 — Likert ═══
st.markdown(f'<div class="section-title">{t("likert_title", lang)}</div>', unsafe_allow_html=True)
st.caption(t("likert_caption", lang))

likert_order = list(LIKERT5_MAP[lang].values())
rows = []
for col, label in LIKERT_QUESTIONS[lang].items():
    lbl = col + "_label"
    if lbl in df.columns:
        vc = df[lbl].value_counts(normalize=True).mul(100).round(1)
        for cat in likert_order:
            rows.append({"Dimension": label, "Response": cat, "Pct": vc.get(cat, 0)})

if rows:
    ldf = pd.DataFrame(rows)
    fig = px.bar(ldf, x="Pct", y="Dimension", color="Response", orientation="h",
                 barmode="stack", color_discrete_sequence=LIKERT_COLORS,
                 category_orders={"Response": likert_order},
                 labels={"Pct":"%","Dimension":""})
    fig.update_layout(legend=dict(orientation="h", y=-0.18, x=0),
                      margin=dict(t=10,b=90), height=440,
                      plot_bgcolor="white", paper_bgcolor="white")
    st.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 5 — Autonomy ═══
st.markdown(f'<div class="section-title">{t("autonomy", lang)}</div>', unsafe_allow_html=True)
c1,c2 = st.columns(2)

if "decisions_label" in df.columns:
    dc = df["decisions_label"].value_counts().reset_index(); dc.columns=["r","n"]
    fig = px.bar(dc, x="n", y="r", orientation="h", color_discrete_sequence=["#1a7f5a"],
                 labels={"r":"","n": t("responses", lang)})
    fig.update_layout(title=t("decisions_t", lang), plot_bgcolor="white", margin=dict(t=40,b=10), height=250)
    c1.plotly_chart(fig, use_container_width=True)

if "exam_label" in df.columns:
    ec = df["exam_label"].value_counts().reset_index(); ec.columns=["r","n"]
    fig = px.bar(ec, x="n", y="r", orientation="h", color="r",
                 color_discrete_sequence=LIKERT_COLORS, labels={"r":"","n":""})
    fig.update_layout(title=t("exam_t", lang), showlegend=False, plot_bgcolor="white",
                      margin=dict(t=40,b=10), height=250)
    c2.plotly_chart(fig, use_container_width=True)

c1,c2 = st.columns(2)
if "epi_label" in df.columns:
    ep = df["epi_label"].value_counts().reset_index(); ep.columns=["r","n"]
    fig = px.pie(ep, names="r", values="n", hole=0.4,
                 color_discrete_sequence=["#1a7f5a","#d63031","#f6c344","#57bb8a"])
    fig.update_layout(title=t("epi_t", lang), margin=dict(t=40,b=10), height=280)
    c1.plotly_chart(fig, use_container_width=True)

if "treat_label" in df.columns:
    tc = df["treat_label"].value_counts().reset_index(); tc.columns=["r","n"]
    fig = px.pie(tc, names="r", values="n", hole=0.4,
                 color_discrete_sequence=["#1a7f5a","#d63031","#cccccc"])
    fig.update_layout(title=t("treat_t", lang), margin=dict(t=40,b=10), height=280)
    c2.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 6 — Clinical ═══
st.markdown(f'<div class="section-title">{t("clinical", lang)}</div>', unsafe_allow_html=True)
c1,c2,c3 = st.columns(3)

if "skin_label" in df.columns:
    sk = df["skin_label"].value_counts().reset_index(); sk.columns=["r","n"]
    fig = px.pie(sk, names="r", values="n", hole=0.4,
                 color_discrete_sequence=["#1a7f5a","#57bb8a","#f6c344","#d63031","#cccccc"])
    fig.update_layout(title=t("skin_t", lang), margin=dict(t=40,b=10), height=280)
    c1.plotly_chart(fig, use_container_width=True)

if "bf_label" in df.columns:
    bf = df["bf_label"].value_counts().reset_index(); bf.columns=["r","n"]
    fig = px.bar(bf, x="n", y="r", orientation="h", color_discrete_sequence=["#2563eb"],
                 labels={"r":"","n":""})
    fig.update_layout(title=t("bf_t", lang), plot_bgcolor="white", margin=dict(t=40,b=10), height=280)
    c2.plotly_chart(fig, use_container_width=True)

if "induce_label" in df.columns:
    ind = df["induce_label"].value_counts().reset_index(); ind.columns=["r","n"]
    fig = px.pie(ind, names="r", values="n", hole=0.4,
                 color_discrete_sequence=["#1a7f5a","#f59e0b","#cccccc"])
    fig.update_layout(title=t("induce_t", lang), margin=dict(t=40,b=10), height=280)
    c3.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 7 — Satisfaction ═══
st.markdown(f'<div class="section-title">{t("satisfaction", lang)}</div>', unsafe_allow_html=True)
if "expect_label" in df.columns and "satisfaction_label" in df.columns:
    c1,c2 = st.columns(2)
    q_order = QUALITY_ORDER[lang]
    for col, field, title in [(c1,"expect_label", t("expect_t", lang)),
                               (c2,"satisfaction_label", t("actual_t", lang))]:
        vc = df[field].value_counts().reindex(q_order, fill_value=0).reset_index()
        vc.columns=["r","n"]
        fig = px.bar(vc, x="r", y="n", color="r", color_discrete_sequence=QUALITY_COLORS,
                     labels={"r":"","n": t("responses", lang)},
                     category_orders={"r": q_order})
        fig.update_layout(title=title, showlegend=False, plot_bgcolor="white",
                          margin=dict(t=60,b=10), height=300)
        col.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 8 — Emotions ═══
st.markdown(f'<div class="section-title">{t("emotions", lang)}</div>', unsafe_allow_html=True)

# emotion column from API = space-separated codes e.g. "1 4 11"
emotion_labels = EMOTION_LABELS[lang]
pos_emotions = POSITIVE_EMOTIONS[lang]

if "emotion" in df.columns:
    counts = parse_multiselect_counts(df["emotion"], list(emotion_labels.keys()))
    rows = []
    for key, label in emotion_labels.items():
        pct = counts[key] / len(df) * 100
        rows.append({"Emotion": label, "Pct": round(pct,1),
                     "Type": t("positive", lang) if label in pos_emotions else t("negative", lang)})
    edf = pd.DataFrame(rows).sort_values("Pct", ascending=True)
    fig = px.bar(edf, x="Pct", y="Emotion", color="Type", orientation="h",
                 color_discrete_map={t("positive", lang):"#1a7f5a", t("negative", lang):"#d63031"},
                 labels={"Pct": t("pct_resp", lang), "Emotion":""})
    fig.update_layout(margin=dict(t=10,b=10), height=380,
                      plot_bgcolor="white", paper_bgcolor="white")
    st.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 9 — Discharge info ═══
st.markdown(f'<div class="section-title">{t("discharge", lang)}</div>', unsafe_allow_html=True)

info_labels = INFO_LABELS[lang]
if "info" in df.columns:
    counts = parse_multiselect_counts(df["info"], list(info_labels.keys()))
    rows = [{"Topic": label, "Pct": round(counts[key]/len(df)*100, 1)}
            for key, label in info_labels.items()]
    idf = pd.DataFrame(rows).sort_values("Pct")
    fig = px.bar(idf, x="Pct", y="Topic", orientation="h",
                 color_discrete_sequence=["#7c3aed"],
                 labels={"Pct": t("pct_info", lang), "Topic":""})
    fig.update_layout(margin=dict(t=10,b=10), height=220, plot_bgcolor="white")
    st.plotly_chart(fig, use_container_width=True)


# ═══ PANEL 10 — Raw data ═══
with st.expander(t("raw_data", lang)):
    hide = [c for c in df.columns if c.startswith("_") or c == "meta/rootUuid"]
    show = [c for c in df.columns if c not in hide]
    st.dataframe(df[show], use_container_width=True, height=400)
    csv = df[show].to_csv(index=False).encode("utf-8")
    st.download_button(t("download", lang), csv,
                       f"ici_data_{datetime.today().strftime('%Y%m%d')}.csv", "text/csv")
